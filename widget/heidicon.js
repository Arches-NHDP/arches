...
( function(){
    ...
    */
    function readAsJson( response ){ return response.json(); }
    ...
    */
	...
	let p = document.createElement( "P" );
	p.innerHTML ="Es wurden <b> keine </b> heidICON-Objekte zu <b>" + term + "</b> gefunden.";
	frame.appendChild( p );
    ...
    */
	...
	let ok = document.createElement( 'INPUT' );
	ok.setAttribute( "type", "button" );
	ok.setAttribute( "value", "Übernehmen" );
	ok.setAttribute( "id", "easydb-ok-button" );
	ok.classList.add( CSS.button );
    ...
    */
	...
	let thumblink = ressourcen[ 'asset' ][ assetIdx ][ 'versions' ][ 'preview' ];
	let cells = "";
    ...
    */
	...
	if( checkNested( heidICONObject[ objectIdx ][ 'asset' ], 0 , "versions", 'preview' ) ){
	    ...
	    thumblink =  heidICONObject[ objectIdx ][ 'asset' ][ 0 ][ 'versions' ][ 'preview' ];
	}
	...
	if ( checkNested( response[ 'objects' ], recordIdx, '_standard', 'eas', 1, objectIdx ) ){
		...
		if( checkNested( heidICONObject[ objectIdx ][ 'asset' ], 0 , "versions", versions[ versionsIdx ] ) ){
			...
			let object ="";
			if( title ){ object += '<b> Titel: </b> ' + title + "<br>"; }
			if( artist ){ object += '<b> Künstler/Urheber: </b> ' + artist + "<br>"; }
			if( date ){ object += '<b> Datum: </b>' + date + "<br>"; }
			if( location ){ object += '<b> Aufbewahrung-Standort: </b>' + location + "<br>"; }
			if( signature ){ object += '<b> Inv.Nr./Signatur: </b>' + signature + "<br>"; }

			let content ="";
			if( object ){ content = object + '<hr>'; }

			let recording = "";
			if( time ){ recording += '<b> Aufnahmezeitpunkt: </b>' + time + "<br>"; }
			if( fotomaker ){ recording += '<b> Fotograf/ Urheber: </b>' + fotomaker + "<br>"; }
			if( license ){ recording += '<b> Rechtsstatus: </b>' + license + "<br>"; }
			if( right ){ recording += '<b> Rechteinhaber: </b>' + right + "<br>"; }
			if( creditline ){ recording += '<b> Creditline: </b>' + creditline + "<br>"; }
...
( function() {
    ...
    */
	...
	if ( assetDeepLink ) {
		...
		p.innerHTML = "<h4> heidICON Objekt </h4>"
		    + "link: "      + assetDeepLink.link           + "<br>"
		    + "thumblink: " + assetDeepLink.thumblink      + "<br>"
		    + "object: "    + assetDeepLink.object         + "<br>"
		    + "ressource: " + assetDeepLink.ressource      + "<br>"
	            + "record: "    + assetDeepLink.recordIdx      + "<br>"
		    + "json: <pre>" + json + "</pre>";
	    }
	}
    }
